"""
PPTX Designer V3 - Korrigierte Version
"""
...
class PPTXDesignerV3:
    """Intelligente PPTX-Praesentation-Engine."""
    ...
    def _add_slide_number(self, slide, number):
        if self.include_slide_numbers:
            self._add_text_box(slide, 12.5, 7.0, 0.7, 0.3, f"{number}/{self.total_slides}", 
                            font_size=10, font_color=self.palette["text_light"], align=PP_ALIGN.RIGHT)
    
    def _add_footer(self, slide):
        if self.company_name:
            self._add_text_box(slide, 0.5, 7.0, 4, 0.3, self.company_name, 
                            font_size=10, font_color=self.palette["text_light"])
    ...
    def _render_executive_summary(self, slide, data, number):
        ...
        if highlights:
            ...
            for highlight in highlights[:4]:
                self._add_text_box(slide, 7.3, y_pos, 4.8, 1.0, f"* {highlight}", font_size=13, font_color="#FFFFFF")
                y_pos += 1.1
    ...
    def _render_persona_slide(self, slide, data, number):
        ...
        for prop in bullets[2:6]:
            self._add_text_box(slide, 3.3, properties_y, 4, 0.4, f"* {prop}", font_size=12, font_color=self.palette["text"])
            properties_y += 0.4
        ...
        for pain in pain_points[:3]:
            self._add_text_box(slide, 8.2, pain_y, 4.4, 0.6, f"* {pain}", font_size=11, font_color="#7F1D1D")
            pain_y += 0.55
        ...
        goal_y = 4.9
        for goal in goals[:3]:
            self._add_text_box(slide, 8.2, goal_y, 4.4, 0.6, f"* {goal}", font_size=11, font_color="#166534")
            goal_y += 0.55
    ...
    def _render_comparison_slide(self, slide, data, number):
        ...
        for col in range(columns):
            ...
            for item in col_bullets[:6]:
                self._add_text_box(slide, x_pos + 0.1, y_pos, col_width - 0.4, 0.7, f"* {item}", font_size=12, font_color=text_color)
                y_pos += 0.8
    ...
    def _render_chart_slide(self, slide, data, number):
        ...
        for bullet in bullets[:5]:
            self._add_text_box(slide, 8.4, y_pos, 4.2, 0.9, f"> {bullet}", font_size=12, font_color="#FFFFFF")
            y_pos += 0.95
    ...
    def _render_timeline_slide(self, slide, data, number):
        ...
        num_phases = min(len(phases), 6) or 4
        phase_width = 12.333 / num_phases
        
        for i, phase in enumerate(phases[:num_phases]):
            x_pos = 0.5 + i * phase_width + phase_width / 2 - 0.25
            color = self.palette["primary"] if i % 2 == 0 else self.palette["accent"]
            self._add_shape(slide, x_pos, 3.35, 0.5, 0.5, color, MSO_SHAPE.OVAL)
            y_offset = 1.8 if i % 2 == 0 else 4.2
            self._add_text_box(slide, x_pos - 1, y_offset, 2.5, 1.2, str(phase), font_size=12, font_color=self.palette["text"], align=PP_ALIGN.CENTER)
    ...
    def _render_conclusion_slide(self, slide, data, number):
        ...
        for i, bullet in enumerate(bullets[:5], 1):
            ...
            self._add_text_box(slide, 0.5, y_pos + 0.05, 0.6, 0.5, str(i), font_size=20, bold=True, font_color="#FFFFFF", align=PP_ALIGN.CENTER)
            self._add_text_box(slide, 1.3, y_pos, 11, 0.9, str(bullet), font_size=16, font_color=self.palette["text"])
            y_pos += 1.1
    ...
    def _render_contact_slide(self, slide, data, number):
        ...
        if bullets or self.company_name:
            ...
            self._add_text_box(slide, 4.2, 4.7, 4.933, 1.6, contact_text, font_size=14, font_color="#FFFFFF", align=PP_ALIGN.CENTER)
    ...
    def create_presentation(self, slides, title="Praesentation", company="", include_sources_slide=False):
        ...
        RENDERERS = {
            ...
            "text": self._render_text_slide,
            "persona": self._render_persona_slide,
            ...
            "conclusion": self._render_conclusion_slide,
            "contact": self._render_contact_slide,
        }
        ...
        for i, slide_data in enumerate(slides, 1):
            ...
            if slide_type not in RENDERERS:
                slide_type = FALLBACKS.get(slide_type, "bullets")
